...
found:
	...
	if (enable) {
		...
		/*  The voltage depends based on the number of fingerprint devices
			present on the device, the devices with two FP devices have only
			1.8V on the regulator so we can't vote this correctly
		*/
...
/**
 * Xiaomi FPC dependencies (Really ugly and unecesarry)
 * Those are just for show to make the FPC HAL happy
 * Our new driver handles all the cases internally.
 *
*/
static ssize_t dummy_set(struct device *dev,
				   struct device_attribute *attr,
				   const char *buf, size_t count)
{
	return count;
}
static DEVICE_ATTR(power_cfg, 0200, NULL, dummy_set);
static DEVICE_ATTR(fingerdown_wait, 0200, NULL, dummy_set);
