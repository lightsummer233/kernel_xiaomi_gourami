				  ...
				  struct device_node *np)
{
	...
	if (val != 0)
		dev_err(dev, "%s: vib_mode not found\n", __func__);
	val = of_property_read_u32(np, "vib_f0_pre", &aw8697->info.f0_pre);
	if (val != 0)
		dev_err(dev, "%s: vib_f0_pre not found\n", __func__);
	val =
	...
	val =
	    of_property_read_u32_array(np, "vib_rtp_time", rtp_time,
				       ARRAY_SIZE(rtp_time));
	if (val != 0)
		dev_err(dev, "%s vib_rtp_time not found\n", __func__);
	...
	if (val != 0)
		dev_err(dev, "%s vib_effect_id_boundary not found\n", __func__);
	val =
	    of_property_read_u32(np, "vib_effect_max",
				 &aw8697->info.effect_max);
	if (val != 0)
		dev_err(dev, "%s vib_effect_max not found\n", __func__);
	...
	for_each_available_child_of_node(np, child_node) {
		...
		rc = of_property_count_elems_of_size(child_node,
						     ...
						     sizeof(u8));
		if (rc < 0) {
			dev_err(dev, "%s Count qcom,wf-pattern property failed !\n",
			       __func__);
		} else if (rc == 0) {
			dev_err(dev, "%s qcom,wf-pattern has no data\n", __func__);
		}
		dev_err(dev, "%s ---%d \n", __func__, __LINE__);
		...
		rc = of_property_read_u8_array(child_node, "qcom,wf-pattern",
					       ...
					       effect->pattern_length);
		if (rc < 0) {
			dev_err(dev, "%s Read qcom,wf-pattern property failed !\n",
			       __func__);
		}
		...
		rc = of_property_read_u32(child_node, "qcom,wf-play-rate-us",
					  &tmp);
		if (rc < 0)
			dev_err(dev, "%s Read qcom,wf-play-rate-us failed !\n",
			       __func__);
		else
		...
		rc = of_property_read_u32(child_node, "qcom,wf-repeat-count",
					  &tmp);
		if (rc < 0) {
			dev_err(dev, "%s Read  qcom,wf-repeat-count failed !\n",
			       __func__);
		} else {
		...
		rc = of_property_read_u32(child_node, "qcom,wf-s-repeat-count",
					  &tmp);
		if (rc < 0) {
			dev_err(dev, "%s Read  qcom,wf-s-repeat-count failed !\n",
			       __func__);
		} else {
		...
		if (tmp > HAP_BRAKE_PATTERN_MAX) {
			dev_err
			    (dev, "%s wf-brake-pattern shouldn't be more than %d bytes\n",
		...
		rc = of_property_read_u8_array(child_node,
					       ...
					       effect->brake, tmp);
		if (rc < 0) {
			dev_err(dev, "%s Failed to get wf-brake-pattern !\n",
			       __func__);
		}
...
static int select_pin_ctl(struct aw8697 *aw8697, const char *name)
{
	...
	for (i = 0; i < ARRAY_SIZE(aw8697->pinctrl_state); i++) {
		...
		if (!strncmp(n, name, strlen(n))) {
			...
			if (rc)
				aw_dev_dbg(aw8697->dev, "cannot select '%s'\n", name);
			else
				aw_dev_dbg(aw8697->dev, "Selected '%s'\n", name);
			goto exit;
		}
	}

	rc = -EINVAL;
	aw_dev_dbg(aw8697->dev, "%s:'%s' not found\n", __func__, name);
...
static int aw8697_hw_reset(struct aw8697 *aw8697)
{
	...
	if (!aw8697->enable_pin_control) {
		if (aw8697 && gpio_is_valid(aw8697->reset_gpio)) {
			...
			aw_dev_dbg(aw8697->dev, "%s pull down1\n", __func__);
			msleep(5);
			gpio_set_value_cansleep(aw8697->reset_gpio, 1);
			aw_dev_dbg(aw8697->dev, "%s pull up1\n", __func__);
			msleep(5);
		} else {
